...
<IMG class="displayed" src="static_figures/amyloid_aggregates.jpg" style="width:404px;height:300px;">
  ...
  server = function(input, output) {
    output$dim_curse <- renderPlot({
      ...
      ggplot(dat, aes(x = n, y = n_max, color = m)) + 
        ...
        scale_y_log10("Całkowita liczba n-gramów (skala logarytmiczna)") +
...
ngram_freq_plot <- table(ngram_freq[["association"]], ngram_freq[["amyloid"]]) %>% 
  ...
  mutate(association = factor(association, labels = c("Amyloidogenny", "Nieamyloidogenny", "Niesprawdzony")),
         pred = factor(pred, labels = rev(c("Predykcja: amyloidogenny", "Predykcja: nieamyloidogenny"))))
