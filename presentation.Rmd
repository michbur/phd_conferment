...
Prace eksperymentalne zazwyczaj poprzedza się analizami komputerowymi, które pozwalają ukierunkować kierunek dalszych badań.
...
* przewidywanie peptydów sygnałowych u eukariontów,    
...
shinyApp(
  ...
  ui = fluidPage(
    ...
    h3("Przykładowe n-gramy"),
    uiOutput("vector_ngrams")
  ),
  
  server = function(input, output) {
    output$vector_ngrams <- renderUI({
      h3(paste0(paste0(gsub("_", "|", decode_ngrams(seq2ngrams(sample_seq, input[["ngram_n"]], aa_v, 
                                                               d = input[["ngram_d"]])[1L:4])),
                       collapse = ", "), ", ..."))
    })
  }
)
...
ggplot(ngram_freq_plot, aes(x = decoded_name, y = amyloid, color = association,
                            label = decoded_name)) +
  geom_text(size = 5) +
  scale_color_manual("Dane eksperymentalne", values = c("red", "darkmagenta"), 
                     labels = c("Amyloidogenny", "Nieamyloidogenny")) +
  scale_y_discrete("Predykcja (AmyloGram)", labels = c("Nieamyloidogenny", "Amyloidogenny")) +
  scale_x_discrete("") +
  my_theme +
